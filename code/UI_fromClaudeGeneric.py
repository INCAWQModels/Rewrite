...
class JSONEditorApp:
    ...
    """Modified save routine with file handling"""
    def save_json(self):
        """Save the updated JSON data back to the original file"""
        updated_data = self.collect_data()
    
        # Use the filename that was originally opened
        ...
        try:
            ...
            self.status_var.set(f"File saved successfully to {filename}!")
...
"""Modified main routine with file dialog"""
def main():
    # Create the root window first but hide it
    root = tk.Tk()
    ...
    if not filename:
        messagebox.showinfo("Cancelled", "No file selected. Application will exit.")
        return
    ...
    # Store the filename in the app instance
